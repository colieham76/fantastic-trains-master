        ...
        if (isSafe) { // if room safe
            ...
            if (creepCarrying < creep.carryCapacity) {
                if (creep.room.name != mineralRoom) { // if not in mineral room
                    ...
                    let keeperLair = creep.pos.findInRange(FIND_STRUCTURES, avoidRadius, { 
                    ...
                    if ((creep.pos.findInRange(FIND_HOSTILE_CREEPS, avoidRadius).length > 0) 
                        || keeperLair && (keeperLair.length > 0)) { // self destroy if not useful damages by NPC
                        actionRunAway.run(creep);
                    }
                    else { // working
                        // find dropped minerals
                        let droppeds = creep.room.find(FIND_DROPPED_RESOURCES, { 
                            filter: c => (c.resourceType != 'energy') });
                        if (droppeds.length > 0) {
                            ...
                            let tombstones = creep.room.find(FIND_TOMBSTONES, { 
                                filter: c => ((_.sum(c.store) > 0) && (c.store.energy == undefined || ((_.sum(c.store) - c.store.energy) > 0))) });
