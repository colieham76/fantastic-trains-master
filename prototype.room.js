...
Room.prototype.updateSpawnQueue = function() {
    ...
    for (let role of listOfRoles) { // calculate number of creeps in current spawn room
        NoOfCreeps[role] = _.sum(imaginaryCreepsInRoom, (c) => c.memory.role == role 
                                 && (c.ticksToLive==undefined||c.memory.spawnTime<c.ticksToLive));
    }
    ...
    if (true) { //(fuckness<1) { // fuckness, 18 equivilient to about 6 HEAL parts or 8 RANGED_ATTACK parts or 18 attack parts
        ...
        if ((creepsInRoom.length < 3) && (room.energyAvailable < 550) 
            && (_.sum(imaginaryCreepsInRoom, (c) => (c.memory.role == 'harvester') 
                      || (c.memory.role == 'pioneer')) < 1 ) 
            && (_.sum(imaginaryCreepsInRoom, (c) => c.memory.role == 'lorry') == 0) ) { // if no harvester or lorries in current spawn room, recovering phase
            ...
            if (room.energyCapacityAvailable >= 350) { // if there is energy to spawn miner
                ...
                if (!Game.rooms[room.name].memory.sourceTravelTime) { // if time takes to travel is undefined
                    ...
                    for (let source of sources) {
                        Game.rooms[room.name].memory.sourceTravelTime[source.id] = 3.14 * calculateTicksBetweenObjects(room.find(FIND_MY_STRUCTURES, { 
                            filter: s => s.structureType == STRUCTURE_SPAWN })[0],source);
                    }
                }
                    ...
                    for (let source of sources) {
                        if (!_.some(imaginaryCreepsInRoom, c => c.memory.role == 'miner'
                            ...
                            && (c.ticksToLive > (3 * 6 + Game.rooms[room.name].memory.sourceTravelTime[source.id])
                        ...
                        { // if there is no miner going to THE source, and life is long enough for the next one to take place or live length is undefined (being spawned)
                            let sourceLink = source.pos.findInRange(FIND_STRUCTURES, 2, {
                                filter: s => s.structureType == STRUCTURE_LINK }); // find a link and determine if it is link or container mining
                    ...
                    let extractor = room.find(FIND_MY_STRUCTURES, { 
                        filter: c => c.structureType == STRUCTURE_EXTRACTOR })[0]; // find extractor already built
                    if (extractor != undefined && extractor.isActive() 
                        && mineral.mineralAmount > 0) {
                        if (!_.some(imaginaryCreepsInRoom, c => c.memory.role == 'miner'
                            && c.memory.sourceID == mineral.id)) {
                            let mineralContainer = mineral.pos.findInRange(FIND_STRUCTURES, 1, { 
                                filter: s => s.structureType == STRUCTURE_CONTAINER });
                            if (mineralContainer.length > 0) {
                                console.log(this.name + ' added mineral miner')
                                spawningQueue.push({ memory: { role:'miner',
                                        sourceID: mineral.id,
                                        target: undefined,
                                        currentRCL: currentRCL,
                                        ifMineEnergy: false, 
       ...
       // if (true) { // no energy problem, start other creeps building
            for (let role of listOfRoles) {
                if (NoOfCreeps[role] < roomSpawningInfo.minCreeps[role]) { // loop through every roles and check with min number. if not enough, spawn other creeps
                    ...
                    spawningQueue.push({
                        memory: { 
                            energy: creepEnergyCost,
                            role: role,
                            target: room.name,
                            RCL: currentRCL,
                            energyMax: room.energyCapacityAvailable},
    ...
    //    }
        ...
        //if (true) { //}(spawningCreepName == undefined) { // if no creeps needed to spawn for the current room
            if (roomSpawningInfo.remoteMode == 0) { // 1 for remote miner mining, 0 for remote harvester mining
        ...
        if (middleRoomName) { // if it is defined
            ...
            if (middleRoom) {
                ...
                if (invaderNum == 0) { // if no invader
                    let noOfKeeperLairLorry = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom, 
                        (c) => c.memory.role == 'keeperLairLorry' 
                        && c.memory.target == middleRoomName);
                    ...
                    for (let source of sources) { // only spawn a miner if it cannot be found both in the spawn room and destination room
                        if (middleRoom.memory.sourceTravelTime) {
                            if (middleRoom.memory.sourceTravelTime[source.id]) {
                                ...
                                if (!_.some(allMyCreepsWithSpawningAndQueueInCurrentRoom,
                                    c => c.memory.role == 'miner' && c.memory.sourceID == source.id
                                        && (c.ticksToLive > (middleRoom.memory.sourceTravelTime[source.id]) * 4 / 5 
                                            || c.ticksToLive == undefined))) {
                                ...
                                middleRoom.memory.sourceTravelTime[source.id] = 3 * calculateTicksBetweenObjects(room.find(FIND_MY_STRUCTURES, {
                                    filter: s => s.structureType == STRUCTURE_SPAWN})[0], source);
                            }
                        } else {
                    ...
                    // pioneer
                    if (_.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom,
                        (c) => c.memory.role == 'pioneer' && c.memory.target == middleRoomName) < 1) { // create pioneer as repairer
                    ...
                    if (noOfKeeperLairLorry < noOfRequiredKeeperLairLorry) {
                        ...
                        spawningQueue.push({
                            memory: {role: 'keeperLairLorry', target: middleRoomName, home: homeName},
                            priority: (rolePriority['keeperLairLorry'] - 1)
                        });
                    } else {
                        if (middleRoom.memory.sourceTravelTime[mineral.id]) {
                            ...
                            middleRoom.memory.sourceTravelTime[mineral.id] = 3 * calculateTicksBetweenObjects(room.find(FIND_MY_STRUCTURES, {
                                filter: s => s.structureType == STRUCTURE_SPAWN})[0], mineral);
                        }
                    }
                } else {
                    console.log('middle room fucked ' + middleRoomName + ' sending invader killers group');
                    let noOfKeeperLairInvaderAttacker = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom, 
                        (c) => c.memory.role == 'keeperLairInvaderAttacker' && c.memory.target == middleRoomName);
                    let noOfKeeperLairInvaderHealer = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom, 
                        (c) => c.memory.role == 'keeperLairInvaderHealer' && c.memory.target == middleRoomName);
                    ...
                    if (noOfKeeperLairInvaderAttacker < 1 && noOfKeeperLairInvaderHealer < 1) {
                        ...
                        spawningQueue.push({
                            memory: {
                                role: 'keeperLairInvaderAttacker',
                                target: middleRoomName,
                                home: room.name,
                                name: keeperLairInvaderAttackerName
                            }, priority: rolePriority['keeperLairInvaderAttacker']
                        });
                        spawningQueue.push({
                            memory: {
                                role: 'keeperLairInvaderHealer',
                                target: middleRoomName,
                                home: room.name,
                                toHeal: keeperLairInvaderAttackerName
                            }, priority: rolePriority['keeperLairInvaderHealer']
                        });
                    } else {
                ...
                let allMyCreepsWithSpawningAndQueueInCurrentRoom = getAllMyCreepsWithSpawning(room.name).concat(spawningQueue);
                // count scouter
                let noOfMiddleRoomScouter = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom, 
                    (c) => c.memory.role == 'scouter' && c.memory.target == middleRoomName);
                //console.log(keeperMiningRoomName,noOfKeeperLairMeleeKeeper)
                if (noOfMiddleRoomScouter < 1) {
                    console.log(room.name + ' creates remote middle mining added scouter ' + middleRoomName);
                    spawningQueue.push({
                        memory: {role: 'scouter', target: middleRoomName},
                        priority: rolePriority['scouter']
                    });
                }
            }
        }
            ...
            if (true) {
                ...
                if (onlyMineralRoomNameAndTime) { // if it is defined
                    for (let onlyMineralRoomName in onlyMineralRoomNameAndTime) {
                        ...
                        if (Memory.rooms[onlyMineralRoomName] 
                            && Memory.rooms[onlyMineralRoomName].hasOwnProperty('isSafe') 
                            && Memory.rooms[onlyMineralRoomName]['invaderCountDown'] 
                            && Memory.rooms[onlyMineralRoomName]['mineralCountDown']) {
                            let mineralRoom = Game.rooms[onlyMineralRoomName];
                            if (mineralRoom) { // room has vision
                                // update invader timer
                                let invaders = mineralRoom.find(FIND_HOSTILE_CREEPS, { 
                            ...
                            Memory.rooms[onlyMineralRoomName] = {
                                'isSafe': true, 'invaderCountDown': 1, 'mineralCountDown': 1 };
                        }

                        // if mineral and invader count down reached, spawn
                        if ((Memory.rooms[onlyMineralRoomName]['mineralCountDown'] - 150 < Game.time) 
                            && (Memory.rooms[onlyMineralRoomName]['invaderCountDown'] - 150 < Game.time)) {
                            Memory.rooms[onlyMineralRoomName]['isSafe'] = true;
                            ...
                            if (checkIfMiddleRoom(onlyMineralRoomName)) {
                                ...
                                let noOfOnlyMineralRoomDefender = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom,
                                    (c) => c.memory.role == 'onlyMineralDefender' && c.memory.target == onlyMineralRoomName);
                                if (noOfOnlyMineralRoomDefender < 1) {
                                    console.log('only mineral ' + onlyMineralRoomName + ' room added defender');
                                    spawningQueue.push({ memory: { 
                                        role: 'onlyMineralDefender', 
                                            target: onlyMineralRoomName, 
                                            home: room.name }, 
                                        priority: rolePriority['onlyMineralDefender'] });
                                }
                            }
                            // spawn miner
                            let noOfOnlyMineralRoomMiner = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom, 
                                (c) => c.memory.role == 'onlyMineralMiner' && c.memory.target == onlyMineralRoomName);
                            if (noOfOnlyMineralRoomMiner < 1) {
                                ...
                                spawningQueue.push({ memory: { 
                                    role: 'onlyMineralMiner', 
                                        target: onlyMineralRoomName, 
                                        home: room.name }, 
                                    priority: rolePriority['onlyMineralMiner'] });
                            }
                            // spawn mineralHauler
                            let noOfOnlyMineralRoomHauler = _.sum(allMyCreepsWithSpawningAndQueueInCurrentRoom, 
                                (c) => c.memory.role == 'onlyMineralHauler' && c.memory.target == onlyMineralRoomName);
                            if (noOfOnlyMineralRoomHauler < 1) {
                                console.log('only mineral ' + onlyMineralRoomName + ' room added Hauler');
                                spawningQueue.push({ memory: { 
                                    role: 'onlyMineralHauler',
                                        target: onlyMineralRoomName, 
                                        home: room.name },
                                    priority: rolePriority['onlyMineralHauler'] });
                            }
                        }
                        else {
