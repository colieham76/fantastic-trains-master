            ...
            if (creep.room.name == creep.memory.target) { // if in target room work
                ...
                if (creep.memory.working == true && _.sum(creep.carry) == 0) {
                    creep.memory.working = false;
                         ...
                         && ((creep.carry.energy > 0.618*creep.carryCapacity)
                  ...
                  for(const resourceType in creep.carry) {
                        ...
                        if (ifShooterRoom && creep.room.terminal) {
                            ...
                            if (terminal && _.sum(terminal.store) < terminal.storageCapacity * 0.95) { // if terminal is defined
                                ...
                                if (terminal.store[resourceType] > creep.room.memory.mineralThresholds.terminalThreshold[resourceType]) {
                                    ...
                                    if (creep.transfer(terminal, 
                                                       resourceType, 
                                                       Math.min(creep.carry[resourceType],
                                                                creep.room.memory.mineralThresholds.terminalThreshold[resourceType] - terminal.store[resourceType])) == ERR_NOT_IN_RANGE) {
                                        creep.travelTo(terminal);
                                    }
                                }
                            }
                    ...
                    if (droppedMinerals.length) { // if there is mineral dropped
                        ...
                        if (tombstones.length > 0) {
                            ...
                            if ((ifMineral)
                                && (ifMineral[0] > creep.carryCapacity)) { // if there is a mineral container and its mineral amount is higher than creep's carrying capacity
                                ...
                                let terminal = creep.room.terminal;
                                /*if ((terminal!=undefined)&&((terminal.store.energy > 50000
                                && creep.room.storage.store.energy < 20000)||(creep.room.storage.store.energy < 5000))) {
                                    ...
                                    if (!hasJobToDo) {
                                        if (creep.room.memory.forLab && creep.ticksToLive > 50) {
                                            ...
                                            for (let labId of outLabIds) {
                                                ...
                                                if (((creep.room.memory.upgradeLabId == undefined) 
                                                     || (labId != creep.room.memory.upgradeLabId)) 
                                                    &&
