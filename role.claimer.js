...
module.exports = {
    run: function (creep) {
        if (creep.memory.prepareToDie && creep.memory.prepareToDie == true) {
            ...
            if (baseFlag) {
            Game.rooms[creep.memory.target].createConstructionSite(baseFlag.pos.x, baseFlag.pos.y, STRUCTURE_SPAWN,
                                                                   'Spawn_' + creep.memory.target + '_1')
            }
            else {
            ...
            if (creep.room.name == creep.memory.target) { // if in target room
                ...
                if (creep.room.controller.my) {
                    ...
                    creep.signController(creep.room.controller, 'Yeeeooooowwww!!');
                }
            }
